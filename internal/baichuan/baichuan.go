...
type GenerateSOAPResponse struct {
	...
	TableData map[string]string `json:"table_data,omitempty"`
}
...
// GenerateMedicalRecordWithTable 调用Baichuan API的/generate_medical_record端点，返回完整响应包含table_data
func GenerateMedicalRecordWithTable(dialogue string, history string) (*GenerateSOAPResponse, error) {
	// 准备请求数据
	requestData := GenerateSOAPRequest{
		Dialogue: dialogue,
		History:  history,
	...
	// 构建/generate_medical_record端点URL
	apiURL := "http://localhost:8000/generate_medical_record"
	...
	// 检查响应状态
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("服务返回错误状态: %s", resp.Status)
	}
	...
	err = json.Unmarshal(respBody, &baichuanResp)
	if err != nil {
		return nil, fmt.Errorf("解析Baichuan响应失败: %v, 响应内容: %s", err, string(respBody))
	}

	// 检查响应状态
	if baichuanResp.Status != "success" {
		return nil, fmt.Errorf("Baichuan服务返回失败状态: %s", baichuanResp.Status)
	}

	return &baichuanResp, nil
}
